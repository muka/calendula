...
export const doTests = async () => {
  ...
  const res = await agent.run(
    'Create a directory called test as working dir. Create a file helloworld.txt with no content. Add a new file hello2.txt with ciao in it. List all available files. ',
  );
  agent.log(res);
};
